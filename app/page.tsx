...
function cloneForm(form: RecipeForm): RecipeForm {
  return { ...form };
}
...
export default function Home() {
    ...
    const init = () => {
      ...
      if (restored) {
        ...
        if (!restoredToastShown.current) {
          ...
          const msg =
            (savedLang === "ru" ? TRANSLATIONS.ru : TRANSLATIONS.en).restored_toast;
  ...
  function loadExample(index: number) {
    ...
    const name = String(
      (lang === "ru" ? ex.displayName_ru : ex.displayName) ?? ex.displayName ?? form.recipe_id
    );
    showToast(t("example_loaded").replace("{name}", name), "info");
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="site-title">
            BreweryX Recipe Generator<span className="accent-dot">_</span>
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        <div className="card">
                ...
                {examplesOpen && (
                  <div className="dropdown-menu right-0 top-full mt-1">
                    {EXAMPLES.map((ex, i) => (
                      ...
                      >
                        {String(
                          (lang === "ru" ? ex.displayName_ru : ex.displayName) ??
                            ex.displayName ??
                            ex.recipe_id
                        )}
                      </button>
          ...
          <div className="section-label">{t("extra_params")}</div>
